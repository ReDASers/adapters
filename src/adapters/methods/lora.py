...
class LoRA(nn.Module):
    ...
    def set_p(self, p:float):
        if self.location == "selfattn":
            ...
            self.std = math.sqrt((2*self.mu)/(max(self.connections_out, self.batches_per_epoch))) # out is just number of neurons for scaling vector
            self.mu -= self.std
            ...
            self.lbound = max(0, 1 - p, self.mu - self.std * 5)
            self.ubound = 1.0
