        ...
        if self.training:
            if self.epoch == 1:
                ...
                w = self.rescale(weights=weights, 
                                sigma=self.sigma_w, 
                                skip_prob=1 - self.p if self.location == "selfattn" else 1.0-1.0/self.batches_per_epoch+1e-6, # here we use 1 - p since we want to skip a lot and high p is confusing
