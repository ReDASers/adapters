...
class LoRA(nn.Module):
    ...
    def set_p(self, p:float):
        if self.location == "selfattn":
            ...
            self.lbound = max(0, self.mu - 2 * self.std)
            self.ubound = min(1.0, max(self.lbound, 1-(1/self.batches_per_epoch)*p, min(1.0,self.mu+self.std+1/(math.sqrt(3)*self.batches_per_epoch))))
